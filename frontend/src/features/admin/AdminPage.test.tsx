...
describe('AdminPage — TENANT_ADMIN', () => {
  ...
  it('creates a user', async () => {
    ...
    await userEvent.type(screen.getByLabelText('Email'), 'new@t.test');
    await userEvent.type(screen.getByLabelText('Password'), 'P@ssw0rd!');
    await userEvent.type(screen.getByLabelText('First name'), 'Ne');
    await userEvent.type(screen.getByLabelText('Last name'), 'W');
    // open the role Select (mouseDown opens AntD selects) and pick MLRO
