...
import com.vyttah.goaml.persistence.shared.JurisdictionRepository;
 ...
 *   <li>insert the {@link TenantEntity} row in {@code public.tenant}</li>
...
public class TenantProvisioningService {
    ...
    public TenantEntity provision(TenantProvisioningRequest request) {
        validate(request);
    ...
    protected TenantEntity persistTenantAndAdmin(TenantProvisioningRequest req,
                                                 UUID id, String schemaName) {
        TenantEntity tenant = new TenantEntity(
                id, req.slug(), req.name(), req.jurisdictionCode(), schemaName, "ACTIVE");
        tenantRepository.save(tenant);

        RoleEntity adminRole = roleRepository.findByName(INITIAL_ADMIN_ROLE)
